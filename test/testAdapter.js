...
function setupTcpServer(callback) {
    ...
    function sendMessage(socket, message, callback) {
        console.log(new Date().toString() + ':     mbusw-TCP-Device: Send to Master: ' + message.toString('hex'));
        socket.write(message, (err) => {
            console.log(new Date().toString() + ':         mbusw-TCP-Device: Send done');
    ...
    const server = net.createServer( (socket) => {
        console.log(new Date().toString() + ': mbusw-TCP-Device: Connected ' + port + '!');
        ...
        socket.on('data',  (data) => {
            ...
            if (!testSocket) {
                console.log(new Date().toString() + ': mbusw-TCP-Device: Connection was already closed');
                return;
            }
            if (!data) {
                console.log(new Date().toString() + ': mbusw-TCP-Device: Received empty string!');
                return;
            }
            const hexData = data.toString('hex');
            console.log(new Date().toString() + ': mbusw-TCP-Device: Received from Master: ' + hexData);

            if (hexData.substring(0, 4) === '1040') {
                const device = hexData.substring(4, 6);
                console.log(new Date().toString() + ':     mbusw-Serial-Device: Initialization Request ' + device);
                ...
                console.log(new Date().toString() + ':     mbusw-TCP-Device: Request for Class 2 Data ID 1');
                ...
                console.log(new Date().toString() + ':     mbusw-TCP-Device: Request for Class 2 Data ID 2');
                ...
                console.log(new Date().toString() + ':     mbusw-TCP-Device: Request for Class 2 Data ID 3');
                sendBuf = Buffer.from('', 'hex');
                sendMessage(socket, sendBuf);
            } else if (hexData.substring(0, 23) === '680b0b6873fd52ffffff1ff') {
                console.log(new Date().toString() + ':     mbusw-Serial-Device: Secondary Scan found');
                sendBuf = Buffer.from('E5', 'hex');
                sendMessage(socket, sendBuf);
            } else if (hexData.substring(0, 6) === '105bfd' || hexData.substring(0, 6) === '107bfd') {
                console.log(new Date().toString() + ':     mbusw-Serial-Device: Request for Class 2 Data ID FD');
        ...
        socket.on('error', (err) => {
            console.error(new Date().toString() + ': mbusw-TCP-Device: Error: ' + err);
        });
        socket.on('close', () => {
            console.error(new Date().toString() + ': mbusw-TCP-Device: Close');
        });
        socket.on('end',  () => {
            console.error(new Date().toString() + ': mbusw-TCP-Device: End');
        });
    ...
    server.on('listening', () => {
        console.log('mbusw-TCP-Device: Listening');
        callback();
    });
